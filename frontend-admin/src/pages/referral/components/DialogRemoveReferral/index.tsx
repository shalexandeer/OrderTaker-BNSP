...
import { useDeleteReferralCode } from "@/services/Referral/Referral.query";
import { tableEditorState } from "@/store/table_recoil";
import { useQueryClient } from "@tanstack/react-query";
import { useRecoilState } from "recoil";

export function DialogRemoveReferral() {
  const [table, setTable] = useRecoilState(tableEditorState);
  const queryClient = useQueryClient();
  
  const referralMutation = useDeleteReferralCode({
    onSuccess: () => {
      toast({ title: "Berhasil menghapus referral", style: { backgroundColor: 'green' } });
  ...
  const { mutate: deleteReferral, isPending } = referralMutation;
  const handleDelete = () => {
  ...
  return (
    <Dialog open={table.isDeleting} onOpenChange={() => setTable((prevTable) => ({ ...prevTable, isDeleting: false, selectedId: '' }))}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Hapus referral {table.selectedItem.name}</DialogTitle>
          <DialogDescription>
